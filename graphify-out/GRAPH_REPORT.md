...
## Corpus Check
- 313 files · ~220,223 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 921 nodes · 1860 edges · 87 communities (54 shown, 16 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 76 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `293f88fb`
...
Cohesion: 0.16
Nodes (27): block_task(), claim_task(), create_task(), get_checkpoint(), get_task(), list_tasks(), _now_iso(), Connection (+19 more)
...
Cohesion: 0.13
Nodes (24): Add-NewProfile(), Expand-TeamMcpPlaceholders(), Format-CardRow(), Format-VisibleRight(), Format-VisibleText(), Get-DesktopBatchAllocation(), Get-EnrichedProfileRows(), Get-ValidatedProfilePath() (+16 more)
...
Cohesion: 0.21
Nodes (16): FastAPI, get_worker(), list_workers(), _now_iso(), Connection, get, post, Register or update an AI worker node / profile. (+8 more)

### Community 18 - "routes_jobs.py"
Cohesion: 0.21
Nodes (16): create_job(), get_job(), get_job_metrics(), list_jobs(), _now_iso(), Connection, get, post (+8 more)
...
Cohesion: 0.26
Nodes (13): CheckpointSubmit, ContextUpsert, JobCreate, JobUpdate, MemoryCreate, BaseModel, QAReviewResponse, QAReviewSubmit (+5 more)

### Community 23 - "config.py"
Cohesion: 0.17
Nodes (8): HTTPAuthorizationCredentials, Verify that incoming request provides a valid API token via Bearer header or…, verify_api_key(), BaseModel, Settings, fixture, Path, setup_test_db()
...
Cohesion: 0.29
Nodes (5): Periodic self-healing supervisor loop: 1. Identifies workers with stale…, run_supervisor_cycle(), test_supervisor_dead_worker_recovery(), Test that tasks with expired leases can be claimed by a new worker or…, test_expired_lease_reclamation_and_recovery()
...
Cohesion: 0.36
Nodes (7): validate_security_settings(), lifespan(), Background supervisor monitoring heartbeats, dead workers, and auto-scheduling…, run_supervisor_loop(), test_development_allows_empty_auth_key(), test_production_accepts_strong_auth_key(), test_production_rejects_missing_auth_key()
...
- **Why does `ClaudeDesktopProxyAdapter` connect `ClaudeDesktopProxyAdapter` to `BaseWorkerAdapter`, `AsyncClient`, `ClaudeDesktopCDPAdapter`, `test_e2e_pipeline.py`?**
...
- **Why does `IApplicationView` connect `IApplicationView` to `IVirtualDesktop`, `Desktop`, `VirtualDesktop11-24H2.cs`, `Guid`, `IApplicationViewCollection`, `IVirtualDesktopPinnedApps`?**
