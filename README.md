...
concurrent instances using `.claude-profiles` are running, the launcher asks once
before proceeding so an accidental isolated launch does not destroy active work.
Use `-ForceIsolated` to skip the confirmation in a deliberate scripted action.
