...
Distributed via `sync.ps1`: new entries from `orchestrator-state/memory/`
are automatically appended under date headers during sync (via
`Sync-MemoryToTeamMemory`), and hand-written entries can also be added directly.
`read_team_memory` provides chat-callable querying across all entries.
...
## 2026-09-02
- [Aaradhya, 2026-09-02T10:17:49Z] Track 21 (Continuous/Streaming Audit Mode, task_2026-09-02_003, parent task_2026-09-02_002 Phase-2 sprint) — DONE, checkpoint submitted. Recommendation: two-layer design — Layer 1 = existing one-shot weekly audit unchanged (Fairlearn+AIF360, χ² α=0.05, bootstrap B≥1000, n≥30 guard); Layer 2 = CUSUM control chart over per-window Core-Four point estimates for trend/drift detection. Rejected per-record sequential testing (violates n≥30, Type-I inflation under naive continuous monitoring) and FairCanary's QDD metric (would add a 5th non-Core-Four metric, breaking M1 lock). New additive-only `DriftObservation` dataclass proposed — references MetricResult by value, does NOT modify the locked schema. Two-tier alerting: Watch (CUSUM alone) vs Alarm (CUSUM + fresh within-window χ² significance, dual-gate). Formal Lan-DeMets alpha-spending flagged as optional separate follow-on, not required for v1. Full deliverable: research/results/21_streaming_audit_mode.md. Relevant to any other Stream G/H/I track touching MetricResult or the report/alerting layer — flag here rather than re-deriving.
- [user10, 2026-09-02T10:17:44Z] BiasAperture Phase-2, Track 29 (task_2026-09-02_011, Pluggable Fairness-Metric Registry, Stream I) — DONE, checkpoint submitted. Verdict: PROTOTYPE FIRST, novelty Moderate, scope CORE EXTENSION. Design: additive fairness/registry.py (MetricSpec: compute_fn/transform[difference|ratio]/fair_point/value_range/eligibility) + new ExtendedMetricResult (metric_name: str, shares NFR-003 guard via extracted helper) — schema.py's locked Literal untouched. Key finding for other tracks: fairness/statistics.py's bootstrap-CI + chi-squared are already metric-agnostic; only eligible_groups() (base.py), the DIR-tolerance branch (backends.py), and the hardcoded [0,1] CI clip (must become declared value_range) need generalizing. Coordination flags raised: Track 26 owns the new "Extended Metrics" report section (not designed here); Track 31/32 should reserve a metric-selection field in their profile/API schemas; Track 36 — plugin metrics must render regulatory-unmapped, never inherit Core Four's EU AI Act tags. Orthogonal to (dropped) Track 23 — MetricSpec has no dependency on FairFace-specific demographic semantics. Full report: results/29_pluggable_metric_registry.md (delivered as file, not committed to repo — research-only task, no repo edits made).
- [user11, 2026-09-02T10:18:32Z] Track 24 (task_2026-09-02_006, Bias Root-Cause Clustering) done — result in checkpoint result_text. Two coordination flags for other tracks:

1. → Track 25/26 owner (Web Dashboard / Drill-Down Report UX): reserve a UI slot for a future "Shared Drivers / Root Cause Themes" section when finalizing the 3-tier drill-down structure — don't lock the layout without it, retrofitting later is harder. Not required now (Track 24's own recommendation is PROTOTYPE FIRST on a lightweight variant only), just don't design it out.

2. Track 24 effectively re-opens Phase-1 Track 16 (Proxy Variable Detection via SHAP, research/results/stream_d_explainability.md) — that pipeline (DeepSHAP + face-parsing segmentation → per-region SHAP-mass vectors) was scoped but never implemented; 6 open items still logged unresolved there. Track 24's "cluster attribution vectors" ask depends on Track 16 existing first. Recommend Aaradhya/Tisha jointly decide whether to revive Track 16 before any Phase-2 track builds further explainability features on top of it.

Also flagging a pre-existing gap unrelated to clustering itself: explainability.py's ExplanationResult output has zero integration into report/generator.py today (verified via grep) — only reachable via `cli.py --explain` stdout. Any future explainability-facing report section (Track 24, 26, or otherwise) needs this wiring built regardless of which feature lands first.

Full report: results/24_bias_rootcause_clustering.md (suggested path, not yet written to repo — research-only per task spec).

## 2026-09-02
- [20, 2026-09-02T10:40:53Z] BiasAperture Phase-2, Track 30 (task_2026-09-02_012, Model-Agnostic Classifier Adapter Layer, Stream I) — DONE, checkpoint submitted. Verdict: PROTOTYPE FIRST, novelty Low-Moderate, scope CORE EXTENSION (InProcessInterface). Design: shared TaxonomyMapper (config-driven mapping from a third-party model's native output labels to the locked race_7/gender_2/age_9 Literal, fail-fast construction-time validation, sum-based probability collapse for many-to-one mappings) consumed by two thin adapters: ONNXRuntimeAdapter (format-agnostic, integrator supplies preprocessing + positional label order — highest effort, silent-reorder risk) and HFPipelineAdapter (AutoModelForImageClassification only, built-in preprocessing, string id2label labels — lower effort, safer mapping authoring). Recommend prototyping HF adapter first to validate the InProcessInterface+TaxonomyMapper contract end-to-end, then ONNX for real client models. Out of scope: models with structurally incompatible native demographic taxonomies — not adapter-solvable, needs Track 23. Coordination flag for board owner: Track 23 is marked "(dropped)" in Track 29's memory entry, but this task's own spec still names Track 23 as the explicit prerequisite for the out-of-scope case — discrepancy surfaced, not resolved here. Full report: results/30_model_agnostic_adapters.md (delivered as file, not committed to repo — research-only task, no repo edits made).
- [Aaradhya, 2026-09-02T10:43:13Z] BiasAperture Phase-2 orchestrator board (parent task_2026-09-02_002) — account-naming inconsistency: several executors claimed/submitted tasks under bare-number account names instead of the "userN" convention used elsewhere on this board. Specifically: Track 26 (task_2026-09-02_008) done under account "24"; Track 30 (task_2026-09-02_012) done under account "20"; Track 32 (task_2026-09-02_014) done under account "24"; Track 37 (task_2026-09-02_019) claimed (not yet done) under account "23". No orchestrator tool exists to rename/relabel owner_account on a task after claim_task or submit_checkpoint has been called — release_task + re-claim under the correct name is the only in-band fix, and that requires the original claiming account to act (an orchestrator/different account can't do it on their behalf). For the 3 already-done tasks (26/30/32), there is currently no way to relabel provenance at all short of a manual edit to the underlying task store. Flagging this as a known, currently-unresolved provenance inconsistency on this board for anyone doing the eventual merge_results/synthesis pass.
- [user25, 2026-09-02T10:43:38Z] Track 31 (task_2026-09-02_013, Config-Driven Audit-as-Code Profiles, Stream I) — DONE, checkpoint submitted. Verdict: PROTOTYPE FIRST, novelty Low-Moderate, scope ADDITIVE (new config layer in front of cli.py flags, no changes to fairness/statistics.py, schema.py's locked Literal, or base.py). Schema: profile_id/extends/regulatory_mapping/metrics(core+extended+active)/subgroups/statistics(alpha, bootstrap_resamples, n_min_guard)/drift_monitoring/overrides[]/ci_cd. Loosening NFR-001(alpha)/NFR-002(bootstrap floor)/NFR-003(n>=30) requires an overrides[] entry (justification+approved_by) plus an auto-stamped append-only audit log line; tightening never needs approval. Validation split into a structural pass + NFR-gate pass, both in one profile_validator.py module usable by cli.py at run-time and Track 32's CI at commit-time — single source of truth. Coordination: Track 29's `metrics.active` selection field is implemented as asked; Track 36's rule enforced at validator level (metrics.extended/plugin entries always render regulatory-unmapped regardless of the profile's top-level regulatory_mapping); drift_monitoring block is optional and orthogonal to Track 21's Layer-2 CUSUM — Track 26 should know extended/plugin metrics need their own unmapped presentation, not folded into the Core Four table. Open items for whoever owns the NFR docs: exact schema.py Core Four enum names, NFR-001's bound direction and NFR-002's exact floor (only current operating defaults α=0.05/B≥1000 were available via team memory, not the source NFR text), and whether an NFR-003 override should still respect a hard absolute floor. Full report: results/31_audit_as_code_profiles.md (delivered as file, not committed to repo — research-only task, no repo edits made).

## 2026-09-05
- [user10, 2026-09-05T04:53:50Z] Completed task_2026-09-04_011 (Gate 3 research, child of task_2026-09-04_006), read-only clone, no edits/commits/push. Submitted full decision brief for items 1-5 via checkpoint (item 6 skipped per spec). Key findings: (1) cli.py::build_parser() confirmed flat argparse, 10 flags, no subcommands — WK4 dev-log's `audit`/`--backend`/`--bca-bootstrap` claims unsupported; --bca-bootstrap would be small-moderate effort (bootstrap fn already accepts n_resamples param), --backend is non-trivial and conflicts with the locked dual-backend-by-design architecture, audit subcommand is a moderate argparse restructure. WP3 PDF-export claim (README:60) also unsupported — zero PDF deps/code. (2) 10,954/97,698=11.2%; 10,954 is the FULL val split (86,744 train + 10,954 val = 97,698, per CLAIM_LEDGER R-002) so this isn't a partial-run shortfall — README already correctly frames it as 10,954/10,954; the "90%" is workstream completion, a separate axis from dataset coverage. Separately, project's own CLAIM_LEDGER "VALIDATED" tier requires a full 97,698-image run, which hasn't happened. (3) Directly viewed architecture_highlevel.jpg — found 5 total stale elements, not the 2 that ledger Cluster C's own table names: YAML-config box and UTKFace-as-live box (Cluster C), SHAP-no-caveat box (filed under Cluster B, same image), plus TWO net-new ones this pass found nowhere in the existing ledger — a 108,501 FairFace figure (stale vs verified 97,698) and an "HTML/PDF" compliance-report box (stale vs PDF-unsupported finding) — neither previously catalogued against this specific image. (4) report/generator.py vs report/templates/generator.py are byte-identical except trailing newline; resolved which is dead: templates/generator.py has zero imports anywhere (also has no __init__.py, isn't even a package) — src/bias_aperture/report/generator.py is the live one (imported by report/__init__.py, cli.py, test_report_generator.py). Note: the task's own literal grep `import.*generator` (lowercase) returns zero hits for BOTH files since real imports use `HTMLReportGenerator` (capital G) — false-negative on the live file too if taken literally; needed case-insensitive re-check to resolve. This closes the "bonus, not scored" open question DISCREPANCY_LEDGER.md Section C had left unresolved. (5) Confirmed no .tex/.md/.docx/.odt source and no staging dir exists in-repo for the two fellowship PDFs (User_Requirement_Document, Literature_Review) — but pdfinfo shows both were authored in Microsoft Word 365 (not Google Docs/LibreOffice — checked raw byte signatures, zero hits), so a .docx source almost certainly existed outside the repo at authoring time; this is "no trackable in-repo source," not "no source ever existed." BiasAperture-AT.md §13 precedent (feasibility_study.pdf, an analogous already-submitted PDF-only gap): resolution pattern was to log gaps as Outstanding Action Item rows rather than hand-edit the PDF, and when the same content need existed in a doc with a live source (requirements.tex), fix it there while leaving the PDF-only doc explicitly, separately outstanding — never force-patched across 3+ tracked revisions. Open note for whoever picks up Gate-3-adjacent work: docs/BiasAperture-AT.md has a duplicate copy at "research/context feed/BiasAperture-AT.md" (flagged under Cluster A, unrelated to this task) — worth deduplicating at some point. Also worth noting for future sessions: this task heavily overlapped with prior task_2026-09-04_001's DISCREPANCY_LEDGER.md (items 1, 2, part of 3 were already covered there) — this pass added value mainly by firsthand-verifying rather than just citing, and by finding the 2 net-new stale image elements + resolving item 4's dead-code question outright.
